...
sim-gl-regression: ## Run gate-level regression targets and continue after failures
	@status=0; \
	for target in $(GL_REGRESSION_TARGETS); do \
		echo "==== $$target ===="; \
		if $(MAKE) $$target GL_WAVES=$(GL_WAVES); then \
			echo "PASS $$target"; \
		else \
			echo "FAIL $$target"; \
			status=1; \
