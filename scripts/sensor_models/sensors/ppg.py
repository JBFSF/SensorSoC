...
# Typical optical signal levels (scaled to 14-bit range)
DC_LEVEL_RED    = 12000
DC_LEVEL_IR     = 14000
AC_AMPLITUDE    = 400
...
def save_validation_plot(t_hr, red, ir, bpm_hr):
    duration_s = 5
...
def process_ppg(t_bpm: np.ndarray, bpm: np.ndarray, output_dir):
    """
    ...
    Args:
        ...
        output_dir: Path to write ppg_digital.csv (e.g. repo_root/sim/data/).

    Returns:
        (red_counts, ir_counts): two uint16 arrays at PPG_ODR_HZ.
    """
    t_hr, ppg_wave, bpm_hr = synthesize_ppg_from_bpm(t_bpm, bpm)
    red, ir                 = apply_adpd144ri_model(ppg_wave)

    save_validation_plot(t_hr, red, ir, bpm_hr)
