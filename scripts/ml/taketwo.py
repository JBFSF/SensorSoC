...
# --- online causal RMSSD (reset per subject) --- NOW just epoch accumulation
...
# time column is in hours, sampled every 30s (0.008333h steps) — convert to seconds
df["time_norm"] = df["time"] * 3600.0
...
df["movement"] /= 8.0
df["delta_hr"] /= 4.0
# df["hr_rmssd"] /= 0.05 switching to just epoch accumulation
